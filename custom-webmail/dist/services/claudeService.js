...
class ClaudeService {
    ...
    createClients(settings) {
        if (settings.provider === 'openai') {
            const options = { apiKey: settings.apiKey };
            if (settings.baseUrl)
                options.baseURL = settings.baseUrl;
            this.openaiClient = new openai_1.default(options);
            this.anthropicClient = null;
        }
        else {
            const options = { apiKey: settings.apiKey };
            if (settings.baseUrl)
                options.baseURL = settings.baseUrl;
            this.anthropicClient = new sdk_1.default(options);
            this.openaiClient = null;
        }
    }
    async reloadSettings() {
        try {
            const stored = await cacheService_1.cacheService.get(SETTINGS_KEY);
            if (stored) {
                this.settings = {
                ...
                this.createClients(this.settings);
                logger_1.logger.info('AI settings reloaded from Redis', { model: this.settings.model, provider: this.settings.provider });
            }
        }
        catch (err) {
            logger_1.logger.warn('Failed to reload AI settings, using current', { error: err.message });
        }
    }
    getSettings() {
...
Output as JSON with fields: summary, category, importance, actionItems (array of strings).`;
        ...
        try {
            ...
            logger_1.logger.warn('Failed to parse AI summary JSON', { text });
        }
        return {
...
Output as JSON with fields: category, importance, labels (array of strings).`;
        ...
        try {
            ...
            logger_1.logger.warn('Failed to parse AI classification JSON', { text });
        }
